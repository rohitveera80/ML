document.addEventListener('DOMContentLoaded', () => {
    ...
    function initHero3D() {
        ...
        const n = 1000; // Separated declaration
        const n2 = n / 2; // Separated declaration
    ...
    if (chartCanvas) {
        ...
        const chartObserver = new IntersectionObserver((entries) => {
            if (entries[0].isIntersecting) {
                demandChart.data.datasets[0].data = [45, 30, 25]; // Final data
                demandChart.update();
    ...
    // --- NLP Roadmap Button Redirection ---
    const nlpRoadmapButton = document.getElementById('nlpRoadmapButton');
    if (nlpRoadmapButton) {
